...
def fast_predict():
    """
    Endpoint para procesar predicciones con Modelo Rápido
    ...
    try:
        ...
        if 'csv_file' in request.files:
            ...
            # Procesar según el tipo de respuesta solicitada
            if return_summary:
                response, status = get_prediction_summary(csv_content, column_mapping, schema, 'fast')
                return jsonify(response), status
            else:
                csv_bytes, status = process_csv_with_predictions(csv_content, column_mapping, schema, 'fast')
